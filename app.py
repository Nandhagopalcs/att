        ...
        if existing_user is None:
            ...
            if credentials['check_id'] == existing_user["_id"] and credentials['check_name'] == existing_user['password']:
                ...
                if existing_user['role']=="admin":
                    return render_template('adminview.html',data=existing_user)
                elif existing_user['role']=="teacher":
                    return render_template('teacherview.html',data=existing_user)
                elif existing_user['role']=="student":
                    return render_template('studentview.html',data=existing_user)
            else:
                return render_template('login.html')
    else:
        return render_template('login.html')
...
def creating():
    if request.method == 'POST':
        ...
        enc=list(encode.pop())
        l=db['login']
        post = {"_id": p_id,"name":name,"c_id":session['college_id'],"c_name":session['college_name'],"password":pwd,"role":"student"}
        l.insert_one(post)
...
def find():
    if request.method == 'POST':
        ...
        postp = {"teacher":tr,"present":present,"absent":absent}
